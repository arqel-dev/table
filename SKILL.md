...
- **12 Column types** em `src/Columns/`: `TextColumn` (limit/wrap), `BadgeColumn` (`colors()` value→token + `icons()` value→lucide name; o `BadgeCell` em `@arqel-dev/ui` mapeia o token de cor para classes Tailwind e renderiza o ícone — valor sem match cai no muted, #50), `BooleanColumn`, `DateColumn` (displayFormat/timezone), `NumberColumn` (decimals/prefix/suffix), `IconColumn`, `ImageColumn` (disk/circular/size), `RelationshipColumn` (`make(name, relation, attribute)` — `toArray()` emite também `display_path` = `{relation}.{attribute}` para o pipeline de export resolver o atributo de exibição, não a relação crua, #152), `ComputedColumn` (`make(name, Closure)`), `SelectColumn` (célula select editável; options/rules/debounce), `TextInputColumn` (célula text-input inline editável; rules/debounce/readonly), `ToggleColumn` (célula toggle editável; onValue/offValue/debounce). Setters comuns: `label/sortable/searchable/hidden/hiddenByDefault/hiddenOnMobile/togglable/alignStart/alignCenter/alignEnd/width/tooltip`. **Payload de alinhamento:** o estado é guardado internamente em `$alignment`, mas `toArray()` emite a chave `'align'` (não `'alignment'`) para casar com o contrato TS do `<DataTable>` (#51).
- **State pipeline** (`getState`/`formatState`): `Column::getStateUsing(Closure)` computa o valor da célula a partir do registro (é o motor do `ComputedColumn`, que não tem atributo de respaldo) e `Column::formatStateUsing(Closure)` envolve o valor cru. Ambos são **honrados de ponta a ponta** — na **serialização para exibição** e na **exportação** (#206): `InertiaDataBuilder::serializeRecord` (em `arqel-dev/core`) injeta `formatState(getState($record), $record)` sob o nome da coluna (a chave que o `<DataTable>` lê, `row[col.name]`), e `ResourceController::serializeColumns` anexa um Closure `state_resolver` ao descritor que os `Csv/Xlsx/PdfExporter` preferem ao `data_get`. O predicado `Column::usesStateResolver()` (true quando há `getStateUsing`/`formatStateUsing`) decide quem entra nesse caminho; uma coluna comum mantém a leitura do atributo cru (sem regressão). `RelationshipColumn` mantém `usesStateResolver() === false` de propósito — usa o seu próprio `display_path`/objeto aninhado (#152), intocado.
- **8 Filter types** em `src/Filters/`: `SelectFilter`, `MultiSelectFilter`, `DateRangeFilter`, `TextFilter`, `TernaryFilter`, `ScopeFilter`, `QueryBuilderFilter` (árvore visual de condições AND/OR resolvida contra `Constraint`s declaradas via `constraints()`), `TrashedFilter` (soft-delete três estados `without`/`with`/`only` — ver abaixo). Setters comuns: `label/apply(Closure)/default/placeholder`.
- **`TrashedFilter` (soft-delete, v0.14.0)** em `src/Filters/TrashedFilter.php`: primitiva first-class para soft-delete (estilo Filament). `TrashedFilter::make(string $name = 'trashed')` cria um filtro de 3 estados — `STATE_WITHOUT` (default, `withoutTrashed()`), `STATE_WITH` (`withTrashed()`), `STATE_ONLY` (`onlyTrashed()`) — que **detecta a trait `SoftDeletes` no model e aplica o scope correspondente ele próprio** (sem `apply()` closure). Num model **sem** `SoftDeletes` é um **no-op inócuo** (a query volta intocada, não 500). Labels customizáveis via `withoutLabel/withLabel/onlyLabel`. **Serializa como `type='select'`** de propósito (`getTypeSpecificProps()` emite `options:[{value,label}]` das 3 states) — renderiza pelo `case 'select'` do `<FilterControl>` existente, sem mudança no React.
...
- **Bulk actions endpoint** (TABLE-008): `POST {panel}/{resource}/bulk/{action}` servido por `arqel-dev/core` (`arqel.resources.bulk` → `ResourceController::bulkAction`, #48/#174 — as rotas standalone `arqel.actions.*` foram removidas), fetcha via `whereIn(getKeyName, ids)` (resolve a PK pela chave real do model, **não** por `'id'` hardcoded, #69), delega para `BulkAction::execute(Collection)` que chunka via `chunkSize(int)` (default 100, clamp ≥ 1). Qualquer bulk action é despachada via `execute()` e recebe uma URL stock convencional mesmo sem callback (#48). `deselectRecordsAfterCompletion(bool)` controla UX pós-execução.
...
- **Valores malformados degradam, não rebentam (#163):** `applyConditions` envolve a chamada `Constraint::apply(...)` num `try/catch (InvalidArgumentException)` — uma shape inválida de valor vinda do payload (ex.: um `between` sem array, ou uma comparação numérica com valor não-numérico) descarta **apenas aquela folha** em vez de propagar um HTTP 500. O resto do filtro continua a aplicar normalmente.
...
use Arqel\Table\Filters\{SelectFilter, DateRangeFilter, TernaryFilter, TrashedFilter};
...
return Table::make()
    ...
    ->filters([
        ...
        TrashedFilter::make(), // 3-state soft-delete (no-op em models sem SoftDeletes)
    ])
...
- Bulk impl: [`packages/core/src/Http/Controllers/ResourceController.php`](../../packages/core/src/Http/Controllers/ResourceController.php) (`bulkAction` — rota `arqel.resources.bulk`)
- ADRs:
